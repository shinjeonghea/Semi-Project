...
<script type="text/javascript">
	...
	function delChannel1(folderNo, channelName){
		
		//alert("2222");
		
        var delChannelForm = document.createElement("form");
        delChannelForm.setAttribute("charset", "UTF-8");
        delChannelForm.setAttribute("method", "Post");  //Post 방식
        delChannelForm.setAttribute("action", "${pageContext.request.contextPath}/DeleteChannelController.do"); //요청 보낼 주소

        var folderNoHidden = document.createElement("input");
        folderNoHidden.setAttribute("type", "hidden");
        folderNoHidden.setAttribute("name", "folderNo");
        folderNoHidden.setAttribute("value", folderNo);
        delChannelForm.appendChild(folderNoHidden);

        var channelNameHidden = document.createElement("input");
        channelNameHidden.setAttribute("type", "hidden");
        channelNameHidden.setAttribute("name", "channelName");
        channelNameHidden.setAttribute("value", channelName);
        delChannelForm.appendChild(channelNameHidden);

        document.body.appendChild(delChannelForm);
        delChannelForm.submit();
	...
	<c:choose>
		<c:when test="${sessionScope.mvo!=null}">
			<li class="nav-item">
				...
				<div id="hello" class="collapse"
					...
					<form method="post"
						...
						<input type="text" name="addfolderName" placeholder="폴더이름"
							size="10" id="addfolderName" required="required"> <input
							type="submit" value="폴더추가" class="btn btn-primary btn-sm">
					</form>
					<form method="post"
						action="${pageContext.request.contextPath}/DeleteFolderController.do"
						onsubmit="return delFolder();">
						<input type="text" name="delfolderName" placeholder="폴더이름"
							size="10" id="delfolderName" required="required"> <input
							type="submit" value="폴더삭제" class="btn btn-primary btn-sm">
					</form>
				</div>
			...
			<li class="nav-item"><a class="nav-link collapsed" href="#"
				...
				<div id="collaps1" class="collapse"
					aria-labelledby="headingUtilities" data-parent="#accordionSidebar">


					<form>
						<input type="button" class="btn btn-primary btn-sm" value="채널추가"
							onclick="addChannel()">
					</form>
					...
					<li class="nav-item">
						...
						<div style="display: inline-block">
							...
							<div style="vertical-align: middle">
								...
								<a class="nav-link collapsed" href="#"<%-- folder 보여w주기 --%> data-toggle="collapse"
						...
						<div id="hello${status.index}" class="collapse"
							...
							<div class="bg-white py-2 collapse-inner rounded">
								<a class="collapse-item" name="${folderlist.folderName}"
									href="javascript:void(0);" onClick="checkAll(this.name);"
									attr-a="onClick:attr-a">전체선택</a>
								<%-- channel보여주기--%>
