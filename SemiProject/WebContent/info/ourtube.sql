...
--유저용 북마크 폴더 가져오기-------------------------------------------------------------------
	...
	select b.folder_no,b.folder_name
	from  bookmark_folder b, member m 
	where b.id=m.id and m.id='java'

	
-- 유저용 채널 가져오기----------------------------------
 select b.folder_name, c.channel_name, c.channel_url
 from bookmark_folder b, channel_member c
 where c.folder_no=b.folder_no and b.folder_no ='5'
